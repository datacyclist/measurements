...
# endless loop, is run in a service file bmp180.service in systemd
  ...
  #print("Pressure    : {0} mbar".format(pressure))
    ...
    while True:
        ...
        if pressure is not None and temperature is not None :
            pressshort = str(round(float(pressure),1))
            tempshort = str(round(float(temperature),1))
            ftempbmp = open('/var/log/bmp180_temperature', 'w')	
            ftempbmp.write(tempshort)
            ftempbmp.close()
            ftempbmp = open('/var/log/bmp180_airpressure', 'w')	
            ftempbmp.write(pressshort)
            ftempbmp.close()
            time.sleep(10)
        else:
            time.sleep(10)
